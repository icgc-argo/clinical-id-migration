...
export async function triggerDataCorrection() {
	...
	for (const donor of donors) {
		...
		for (const tr of donor.treatments) {
			if (tr.clinicalInfo) {
				...
				const therapy = therapies.filter((th) => {
					return (
						th.clinicalInfo.submitter_treatment_id ==
						tr.clinicalInfo.submitter_treatment_id
					);
				});
		...
		if (updateDonor) {
			myConsole.log(
				"Updating therapies in right treatments: " +
					" - " +
					donor.submitterId +
					" - " +
					donor.programId
