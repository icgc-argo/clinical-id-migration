...
export async function beginMigration(response: Response) {
  ...
  for (const donor of donors) {
    ...
    try {
      ...
      myConsole.log(
        "Oops, Croc attack! - migration for donor failed." +
          " -Checkpoint: " +
          checkpoint +
          " -Program Id: " +
          donor.programId +
...
async function migrateComorbidity(donor: ClinicalDonor) {
  ...
  for (const cm of donor.comorbidity) {
    ...
    comorbidity.comorbidityTypeCode =
      cm.clinicalInfo.comorbidity_type_code?.toString() || "-";
...
async function migrateFamilyHistory(donor: ClinicalDonor) {
  ...
  for (const fh of donor.familyHistory) {
    ...
    familyHistory.familyRelativeId =
      fh.clinicalInfo.family_relative_id?.toString() || "-";
...
async function migrateFollowUps(donor: ClinicalDonor) {
  ...
  for (const fl of donor.followUps) {
    ...
    folowUp.submitterFollowUpId =
      fl.clinicalInfo.submitter_follow_up_id?.toString() || "-";
...
async function migratePrimaryDiagnosis(donor: ClinicalDonor) {
  ...
  for (const pr of donor.primaryDiagnoses) {
    ...
    primaryDiagnosis.submitterPrimaryDiagnosisId =
      pr.clinicalInfo.submitter_primary_diagnosis_id?.toString() || "-";
...
async function migrateTreatments(donor: ClinicalDonor) {
  ...
  for (const tr of donor.treatments) {
    ...
    treatment.submitterTreatmentId =
      tr.clinicalInfo?.submitter_treatment_id?.toString() || "-";
...
async function migrateSpecimens(donor: ClinicalDonor) {
  ...
  for (const spc of donor.specimens) {
    ...
    specimen.submitterSpecimenId = !spc.clinicalInfo.submitter_specimen_id
      ? spc.submitterId
      : spc.clinicalInfo.submitter_specimen_id?.toString() || "-";
